      ...
      if (openDropdown !== null) {
        ...
        dropdowns.forEach((dropdown, index) => {
          if (dropdown.contains(event.target) || 
              document.querySelector(`.filter-button-${index}`)?.contains(event.target)) {
            clickedInside = true;
          }
        });
          ...
          <div className="flex items-center gap-2 sm:gap-3 md:gap-4 flex-wrap w-full sm:w-auto">
            {filterOptions.map((filter, index) => (
              <div key={index} className="relative">
                <Button
                  variant="outline"
                  className={`filter-button-${index} flex items-center justify-center gap-1 px-2 sm:px-3 md:px-4 py-1.5 sm:py-2 text-xs sm:text-sm rounded-full border border-solid ${openDropdown === index || filters[filter.filterName] ? 'border-black text-black' : 'border-gray-500 text-gray-500'}`}
                ...
                {/* Filter dropdown */}
                <div className={`filter-dropdown absolute z-10 mt-2 w-48 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 p-2 ${openDropdown === index ? 'block' : 'hidden'}`}>
                  {filter.type === 'text' && (
                    <input
                      type="text"
